...
- git流程：
    ...
    1.gitee上创建仓库，里面可以为空。
    2.本地设置用户签名（首次下载git一定要进行设置，否则后续提交代码会出错，可以设置虚拟邮箱。意义是操作远程仓库后知道是谁操作的）
      git config --global user.name "用户名"
      git config --global user.email 邮箱
    3.在本地自定义文件夹，git init初始化一个仓库 ，并默认带有本地master分支
    3.git remote add origin https://gitee.com/xxxx.git   //本地仓库与远程仓库关联
    4.git add .
    5.git commit -m '内容'  //执行后如果自动格式化代码了，需要再次提交。 
    6.git push -u origin master 


-  命令集合
    git init 初始化仓库，默认带有本地master分支
    git status  查看当前库的状态：处于哪条分支、缓存区是否有内容/本地是否有修改但是未提交的
    git add .  添加所有文件到暂存区；git add 文件名 + 后缀 只添加指定文件
    git branch 查看所有分支
    git checkout -b  xxx   创建一个本地分支并切换
    ...
    (第一次使用加上了 -u 参数，是push内容，远程origin的master分支将和同名的本地分支master关联，之后直接使用git push即可。)
    (需要确保本地分支和远程分支的名称一致)
