...
**子组件获取父组件传递数据:方式1**
  ...
  <!-- <script setup lang="ts"> 此写法常用 -->

  let props = defineProps({     （defineProps其实是声明接收而已,因为js代码都写在setup）
    info:{
      type:String,//接受的数据类型
      default:'默认参数',//接受默认数据
    },
...
-------------------------------------------------------------------------------------------------------------
 ...
 # 自主开发笔记：
  ...
  4.安装 引入element-plus、安装scss，设置全局scss样式及变量（跳）
    注意：1.安装element-plus时引入分页等是英文的，需配置一个国际化，设置为中文

  5.TypeScript 编译配置（跳）、环境变量的配置、SVG图标配置（跳）。
    注意：
      1.vue3中环境变量的写法不同：（注意）
  ...
  9.API接口统一管理
    注意： 1.import type {} 不会导入实际的代码，只会导入类型信息
           2.限制调用接口时，传入参数的类型与结构，后台返回的数据也做限制（特殊，多type.ts）
...
.ts文件在引入文件时，没有声明ts类型，会出现红色波浪线，此时能运行，但是打包也会报错：
解决方法：在此位置前面加一句忽略ts检测

组件页面的js代码，引入组件等等出现红色波浪线,但是能使用，是因为VS的语法提示插件vetur对vue3不太兼容：
解决方法：卸载插件vetur，重启




全局scss样式：新建reset.css清楚默认样式（npm官网搜索，然后复制）
              index.scss文件引入reset.css
             全局变量$，vite不认得会报错，得写在一个scss文件，配置其路径位置给vite。（如果是$单个文件内使用，应该没有这问题）
             less用@


  登录功能：点击让仓库发请求、判断结果返回到组件，组件内提示并调整
        安装pinia(如果vite是4.3.2，不是最新版，pinia也不能安装最新版)
        复习：promise返回值
        仓库需限制ts类型：返回值的地方。ts类型限定文件也修改：?表示可能有也可能没有...
        封装持久化方法


  登录成功：动态提示时间信息，表单校验功能，ref获取组件let 名称=ref()

  一级路由：layout组件布局编写，抽离部分全局样式变量,计算calc()
          左侧菜单logo+标题的组件  
          setting.ts暴露对象设置标题和logo
          左侧导航菜单封装组件：滚动条组件包裹，高度100%-logo组件高度
          左侧导航菜单静态页面
  vue3.2引入组件后,不用注册，因为setup简写在标签上的组合式api写法
...
defineprops方法使用时，页面调用可以直接使用变量即可，无需.（特殊）
区分事件挂载的情况：
1.挂载在原生标签上，为原生事件（留意事件参数）
2.挂载在组件上：若是vue2。。。
...
动态路由：
  将路由数组添加到用户store中，作为用户的菜单属性（记得限制类型）；再去页面引入用户store
  页面获取到数组传值给自定义组件，
  渲染菜单时根据孩子属性是否存在、是否只有一个，区分三种情况；用到递归组件，需按vue2给组件起名字。（各种路由是否有缺陷？）
  并不是所有路由都需要展示，得隐藏部分/过滤数组


菜单图标：
将icon所有图标注册为全局组件，Object.entries：将对象变成键值的数组,每对属性和属性值对应生产一个数组元素
图标动态加载得用到动态组件is属性
菜单的唯一标识设置为path，添加菜单点击事件时(或者添加router属性)，标识即可为跳转路径
...
-  命令集合
  ...
  缓存区：commit提交的区域，可commit部分文件到此，之后再push到远程。
    ...
    (第一次使用加上了 -u 参数，是push内容后，远程origin的master分支再和同名的本地分支master关联，之后直接使用git push即可。)
    (需要确保本地分支和远程分支的名称一致)
