...
### 1.4 vue3的v-model（注意）
  ...
  v-model指令可是收集表单数据(数据双向绑定)，除此之外它也可以实现父子组件数据同步。（注意）
   ...
   v-model同步用法2：(element-plug分页等常用,可以理解为就是一种简写)
        ...
        （特殊：在vue3中一个组件可以通过使用多个v-model,让父子组件多个数据同步,下方代码相当于给组件Child传递两个props分别是pageNo与pageSize，以及绑定两个自定义事件update:pageNo与update:pageSize实现父子数据同步）
...
### 1.5 useAttrs (注意需引入)
  ...
  <script setup lang="ts">
  //引入useAttrs方法:获取组件标签身上属性与事件，前提：属性只能获取到没有被prop接收的。事件接收的待确定。
...
子组件获取父组件实例VC：（子组件得使用方法，父组件数据也得先对外暴露）
...
**3.入口文件导入**
  ...
  4.直接在src/assets/icons文件夹资源中分别创建多个.svg文件，复制官网的多个图标svg代码
 ...
 # 自主开发笔记：
  ...
  10.router配置
    ...
    12.一级路由layout：layout组件布局编写，抽离部分全局样式变量
      ...
      *注意：
          1.注意ts文件中不能只引入pina小仓库，需同时引入大小仓库（有时候不用）
...
待做：导航组件、动态菜单及路由、表格分页封装、主题切换、大屏
