...
父组件内部代码如下:
  ...
  <template>
    <div>
      <Todo :todos="todos">
        <template v-slot="{$row,$index}">  （建议接收方式使用slot-scope，因为当多个具体名插槽v-slot应该用来命名）
          <!--父组件决定子组件的结构与外观-->
...
自主分类笔记：
  ...
  html+css：
    ...
    --webkit-mask属性：（图片中透明的则代表不显示，不透明则表示显示）
      用于创建一个遮罩效果，与背景颜色不同的是，遮罩效果可以创建非常复杂的形状，而不仅仅是简单的颜色填充。
  ...
  TS：（不影响运行，只是代码提示更加严谨）
    ...
    - 限制数组中的每一个元素的格式：
      ...
      export interface TradeMark {  //单个元素
        id?: number
        tmName: string
        logoUrl: string
      }
    ...
    -限制对象格式的复用：
      ...
      2.interface IAdminUser extends IUser {
          isAdmin: boolean;
        }
  ...
  echarts:
    ...
    1.category在散点图可调整均匀，在柱状图中为设置主轴。
