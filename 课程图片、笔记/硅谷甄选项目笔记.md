...
-------------------------------------------------------------------------------------------------------------
 ...
 # 自主开发笔记：
  ...
  10.router配置
      注意：1.vue2,vue3都需要use(xx)，但是use的内容不一样，前者是Router构造函数，后者是router实例；且vue2在new Vue中需要传router实例，vue3不用。
...
安装element-plus时引入分页等是英文的，需配置一个国际化，设置为中文

.ts文件在引入文件时，没有声明ts类型，会出现红色波浪线，此时打包也会报错：
