<template>
    <el-form ref="formRef" :model="stockForm" :rules="rules" label-width="85px">
        <el-form-item label="添加方式">
            ...
            <el-col :lg="12" :span="24">
                <el-radio-group v-model="addWay">
                    <el-radio :label="0" :border=true>新增去向</el-radio>
                    <el-radio :label="1" :border=true>已知去向</el-radio>
                </el-radio-group>
              </el-col>

            <el-col :lg="12" :span="24" v-show="addWay==0">
                <el-text  type="warning"> 注意：新增的去向信息会同步至物资去向</el-text>
            </el-col>

        </el-form-item>

        <el-form-item prop="priority" label="优先级">
            <el-radio-group v-model="stockForm.priority">
                <el-radio v-for="item in ProvidePriorityOption" :label="item.value">{{ item.label }}</el-radio>
            </el-radio-group>
        </el-form-item>
        <el-form-item prop="type" label="领取类型">
            <el-radio-group v-model="stockForm.type">
                <el-radio v-for="item in ProvideTypeOption" :label="item.value">{{ item.label.replace('领取','') }}</el-radio>
            </el-radio-group>
        </el-form-item>
        ...
        <div v-if="addWay == 0">
            ...
            <el-form-item prop="name" label="具体去向">
                <el-col :lg="8" :span="24">
                    <el-input v-model="stockForm.name" placeholder="请输入具体去向名称" />
                </el-col>
        ...
        <div v-else>
            <el-form-item prop="consumerId" label="物资去向">
                <el-select v-model="stockForm.consumerId" placeholder="请选择已知去向" clearable>
                    <el-option v-for="item in consumerAll" :label="item.name" :value="item.id" />
                </el-select>
            </el-form-item>

            <div v-if="consumerSelected">
                <el-form-item label="物资地址">
                    <el-col :lg="8" :span="24">
                        <el-input v-model="consumerSelected.address" disabled />
                    </el-col>
                </el-form-item>
                <el-form-item label="联系方式">
                    <el-col :lg="8" :span="24">
                        <el-input v-model="consumerSelected.phone" disabled />
                    </el-col>
                </el-form-item>
                <el-form-item label="联系人员">
                    <el-col :lg="8" :span="24">
                        <el-input v-model="consumerSelected.contact" disabled />
                    </el-col>
                </el-form-item>
            </div>
        </div>
        ...
        <el-form-item>
            <el-button type="primary" :loading="loading" @click="onSubmit">立即发放</el-button>
...
import type { ProvideForm, productItem, ConsumerItem } from "@/types/api/inventory/provide";
import { ProvideTypeOption,ProvidePriorityOption } from "@/bus/shareData";
import { AddSocksType } from "@/types/api/enum";
import useRegionSelect from "@/hook/common/regionSelect";
import type { FormInstance, FormRules } from 'element-plus'
import $bus from "@/bus/index";
import { provideGood, findAll } from "@/api/inventory/provide";
import Tip from '@/utils/element/elMessageTip'
import { useRouter } from "vue-router";
import {ProvidePriority,ProvideSocksType } from "@/types/api/enum";
...
const rules = reactive<FormRules<{}>>({
    ...
    type: [
        { required: true, message: '请填写领取类型', trigger: 'change' },
    ],
    priority: [
...
let stockForm = ref<ProvideForm>({
    ...
    remark: '',
    sort: 1,
    type: ProvideSocksType.government,
    products: [],
    priority: ProvidePriority.priority2,
...
let consumerAll = ref<ConsumerItem[]>();
let consumerSelected = computed(() => {
    return consumerAll.value?.find((item) => item.id == stockForm.value.consumerId)
})
...
const onSubmit = async () => {
    ...
    formRef.value?.validate().then(() => {  //这种方式不会有控制台警告
        ...
        if (addWay.value == 0) {
            ...
            stockForm.value.consumerId = undefined;
        } else {
            let holdkeys = ['priority','consumerId','type', 'consumerId', 'remark', 'sort', 'products'];
        ...
        loading.value = true;
        provideGood(stockForm.value).then(() => {
            Tip('success', '物资入库成功');
            $router.push('/inventory/provide');
        }).finally(() => {
            loading.value = false;
        })
