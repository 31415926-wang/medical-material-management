<template>
    <CrudTable ref="CrudTableRef" @openDialogBefore="handleopenBefore" :operateColWidth="260" :needOperate="true"
        ...
        <template #search-custom="{ scope }">
            ...
            <el-form-item label="性别">
                <el-radio-group v-model="scope.searchParam.sex">
                    <el-radio :value="1" size="large" style="padding-bottom: 3px;">男</el-radio>
                    <el-radio :value="0" size="large" style="padding-bottom: 3px;">女</el-radio>
                    <!-- <el-radio :value="undefined" size="large" style="padding-bottom: 3px;">全部</el-radio> -->
                </el-radio-group>
            </el-form-item>
        </template>
        ...
        <template v-slot:sexCol="{ scope }">
            <el-tag :type="scope.row.sex === 0 ? 'danger' : 'primary'">{{ GetLabel(scope.row.sex, SEX_OPTIONS)
                }}</el-tag>
        </template>
                        ...
                        <el-form-item label="部门" prop="departmentId">
                            <el-select v-model="formInfo.departmentId" clearable placeholder="请选择部门">
                                <el-option v-for="item in departmentList" :key="Number(item.id)" :label="item.name"
                                    :value="Number(item.id)">
                                    <span style="float: left">{{ item.name }}</span>
                                    <span style="float: right;"><el-text type="primary">{{ item.total
                                            }}人</el-text></span>
                                </el-option>
                            </el-select>
                        </el-form-item>
    ...
    <el-dialog v-model="dialogSetShow" title="分配角色" :before-close="handleSetCloseBefore">
        <div class="custom-scrollbar dialog-container">
            <el-transfer v-model="transferSelected" :data="optionList" filterable filter-placeholder="请输入搜索内容"
                :titles="['可分配角色列表', '已选中的角色列表']" :button-texts="['移除角色', '添加角色']" />
        </div>
        <template #footer>
            <el-button icon="CloseBold" @click="handleSetCloseBefore">
                取消分配
            </el-button>
            <el-button type="primary" icon="Setting" :loading="loadingSetStatus"
                @click="handleSetSumit">确定分配</el-button>
        </template>
    </el-dialog>
</template>
