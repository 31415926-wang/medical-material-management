<template>
    <CrudTable ref="CrudTableRef" :default-tool-bt-list="[]" :dialogWidths="{
        ...
        <!-- 自定义的列 -->
        <template v-slot:stockCol="{ scope }">
            <el-text type="warning">{{ scope.row?.stock }}</el-text>
        </template>
        ...
        <template v-slot:productNumberCol="{ scope }">
            ...
            :min="scope.row.productNumber == 0 ? 0:1" :max="scope.row.stock" 
...
import type { pageRes } from "@/types/api/index";
...
//接口做特殊处理
let newApiMethod=cloneDeep(infoApiMethod);
// @ts-ignore
newApiMethod.getList=newApiMethod.findProductStocks;
